...
def write_kicad(base_path, out_path, segments, vias, layer_map=None,
                via_pad=0.6, via_drill=0.3, strip=False, keep_nets=(),
    ...
    keep_net_ids = {net_to_id[n] for n in keep_nets if n in net_to_id}
    protected_ids = {net_to_id[n] for n in protected if n in net_to_id}
    ...
    if strip:
        ...
        for line in lines:
            ...
            if s.startswith("(segment"):
                ...
                nid = int(m_net.group(1)) if m_net else None
                if nid is not None and (nid in protected_ids):
            ...
            if s.startswith("(via"):
                m_net = re.search(r'\(net\s+(\d+)\)', line)
                nid = int(m_net.group(1)) if m_net else None
                if nid is not None and (nid in protected_ids or nid in keep_net_ids):
                    stripped_lines.append(line)
                else:
                    dv += 1
                continue
        ...
        if verbose:
            print("stripped previous KiCad routing: -%d segment, -%d via" % (dl, dv))

    body = "".join(lines)
    last_paren = body.rfind(")")
    if last_paren == -1:
        raise ValueError("Invalid KiCad PCB file: no closing parenthesis found")

    default_layer_map = {"TopLayer": "F.Cu", "BottomLayer": "B.Cu",
                         "F.Cu": "F.Cu", "B.Cu": "B.Cu",
                         "Inner1": "In1.Cu", "Inner2": "In2.Cu"}
    lm = dict(default_layer_map)
    if layer_map:
        lm.update(layer_map)

    new_blocks = []
    for s in segments:
        l_name = lm.get(s["layer"], s["layer"])
        net_id = net_to_id.get(s["net"], 0)
        new_blocks.append(
            '  (segment (start %.4f %.4f) (end %.4f %.4f) (width %.4f) (layer "%s") (net %d))\n'
            % (s["x1"], s["y1"], s["x2"], s["y2"], s["w"], l_name, net_id)
        )

    for v in vias:
        net_id = net_to_id.get(v["net"], 0)
        new_blocks.append(
            '  (via (at %.4f %.4f) (size %.4f) (drill %.4f) (layers "F.Cu" "B.Cu") (net %d))\n'
            % (v["x"], v["y"], via_pad, via_drill, net_id)
        )

    output_text = body[:last_paren] + "".join(new_blocks) + body[last_paren:]
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write(output_text)
    if verbose:
        print("wrote %s (+%d segments, +%d vias)" % (out_path, len(segments), len(vias)))
...
def main(argv):
    ...
    if fmt == "kicad":
        base = opt("--base")
        if not base:
            raise SystemExit("--format kicad needs --base (a .kicad_pcb file)")
        lm = None
        if "--layer-map" in argv:
            with open(opt("--layer-map"), encoding="utf-8") as fh:
                lm = json.load(fh)
        write_kicad(base, out, segs, vias, layer_map=lm,
                    via_pad=float(opt("--via-pad", 0.6)),
