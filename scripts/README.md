...
```
kicad/       headless DRC, 1-command JLCPCB export, autoroute loop
...
```
   ...
   raw .dsn   ──► routing/dsn_rewrite.py ──► routing/dsn_slim.py ──► your router
                                                                  │
   .ses       ──► routing/ses_import.py (--format kicad) ──► MERGE ┘
   readback   ──► placement/import_kicad.py ──► after.json
   ...
   kicad-cli  ──► kicad/drc_check.py (DRC 0/0 gate)
              ──► kicad/export_jlcpcb.py (Gerber, Drill, CPL, BOM, ZIP)
